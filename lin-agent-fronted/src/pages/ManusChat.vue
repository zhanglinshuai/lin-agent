...
import {openManusSSE} from '@/services/sse'
...
function send() {
  ...
  currentStream?.close()
  currentStream = openManusSSE(
      text,
      (chunk) => {
        const msg = messages.value[idx]
        if (msg && msg.role === 'assistant') msg.content += chunk
        if (msg && msg.role === 'assistant' && msg.loading) msg.loading = false
        nextTick(() => {
    ...
    <div class="layout" :class="{ collapsed: !showSidebar }">
      <aside ref="sidebarRef" class="sidebar" :class="{ collapsed: !showSidebar, peek: sidebarPeek, closing: peekClosing }" @mouseenter="onPeekEnter" @mouseleave="onPeekLeave">
        <div class="sidebar-top">
          <div class="sidebar-header">超级智能体</div>
          ...
          <div class="switch-app">
            <router-link class="switch-link" to="/emotion">
              <span class="switch-logo">💖</span>
              <span class="switch-name">AI 情感大师</span>
            </router-link>
          </div>
      ...
      <div class="content" :class="{ empty: !messages.length }" @click="showAvatarMenu=false" @mousemove="onEdgeHover" @mouseleave="onEdgeLeave">
          ...
          <div class="hero-page" v-else>
            <div class="hero-head">
              ...
              <div class="hero-text">
                <div class="hero-title manus">超级智能体</div>
                <div class="hero-sub">你的 AI 助手，帮你完成各种任务</div>
              </div>
            </div>
          </div>
          <div class="compose-card">
            <div class="placeholder">给 超级智能体 发送消息</div>
